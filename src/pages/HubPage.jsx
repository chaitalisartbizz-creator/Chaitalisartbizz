...
export default function HubPage() {
  ...
  const { masterclasses } = useData();
  const { addToCart, isInCart } = useCart();

  // Use admin-managed masterclasses if available, else fallback to static data
  const courses = (masterclasses && masterclasses.length > 0) ? masterclasses : FALLBACK_COURSES;
  ...
  const handleAddToCart = (course) => {
    addToCart({
      id: `masterclass-${course.id}`,
      name: course.title,
      price: Number(course.price),
      mrp: null,
      img: course.img,
      category: 'Masterclass',
      brand: course.instructor,
      rating: course.rating,
      isMasterclass: true,
    });
  };

  const isEnrolled = (course) => isInCart(`masterclass-${course.id}`);
                ...
                <div className="lg:w-1/2 p-8 lg:p-10 flex flex-col justify-center relative">
                  ...
                  {featured.modules && (
                    <div className="space-y-3 mb-8">
                      {(Array.isArray(featured.modules) ? featured.modules : (featured.modules || '').split('\n').filter(Boolean)).map((mod, i) => (
                  ...
                  <div className="mt-auto flex flex-col sm:flex-row items-center gap-4 pt-6 border-t border-stone-200">
                    <div className="text-3xl font-black text-[#2C2C2C]">₹{Number(featured.price).toLocaleString()}</div>
                    <button
                      onClick={() => handleAddToCart(featured)}
                      className={`w-full sm:w-auto flex-1 font-bold py-3.5 rounded-xl transition-all shadow-xl flex items-center justify-center gap-2 ${
                        isEnrolled(featured)
                          ? 'bg-emerald-600 text-white'
                          : 'bg-[#2C2C2C] hover:bg-[#1A1A1A] text-[#C9A84C]'
                      }`}
                    >
                      <ShoppingBag size={18} />
                      {isEnrolled(featured) ? '✓ Added to Cart' : 'Add to Cart'}
                    </button>
                  </div>
                </div>
              </div>
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <div className="mt-auto flex items-center justify-between pt-4 border-t border-stone-100">
                        ...
                        <button
                          ...
                          className={`font-bold text-xs px-4 py-2 rounded-lg transition-colors flex items-center gap-1.5 ${
                            isEnrolled(course)
                              ? 'bg-emerald-600 text-white'
                              : 'bg-[#F2EDE4] hover:bg-[#C9A84C] text-[#A8873A] hover:text-[#2C2C2C]'
                          }`}
                        >
                          <ShoppingBag size={12} />
                          {isEnrolled(course) ? '✓ Added' : 'Add to Cart'}
                        </button>
                      </div>
                    </div>
                  </div>
