...
function ToggleSwitch({ enabled, onChange, id }) {
  return (
    <button
      ...
      className={`relative inline-flex h-7 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-[#C9A84C] focus:ring-offset-2 ${
...
// ─── Section Card ────────────────────────────────────────────────────────────
function SectionCard({ title, icon: Icon, iconColor = 'text-[#C9A84C]', iconBg = 'bg-[#F2EDE4]/50', children, badge }) {
  return (
...
function InfoBanner({ icon: Icon, children, color = 'blue' }) {
  const colorMap = {
    ...
    amber: { bg: 'bg-gray-200', border: 'border-gray-200', text: 'text-gray-200', icon: 'text-gray-200' },
    ...
    <div className="max-w-3xl animate-fade-in">
      ...
      <ScrollReveal>
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex items-center gap-2 bg-[#C9A84C] hover:bg-[#a65d14] disabled:opacity-70 text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
          >
            <Save size={18} />
      ...
      <div className="space-y-6">
        ...
        <ScrollReveal delay={50}>
          <div className="bg-gradient-to-r from-[#C9A84C]/10 to-[#F2EDE4]/50 border border-gray-200 rounded-2xl p-5 flex gap-4 items-start">
            <div className="p-3 bg-[#C9A84C] rounded-xl shrink-0">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 leading-relaxed">
                ...
                reference ID. Your{' '}
                <span className="font-semibold text-[#C9A84C]">Key ID</span> is safe to expose on
                the frontend; your{' '}
        ...
        {onlinePaymentEnabled && (
          <ScrollReveal delay={150}>
            <SectionCard
              title="Gateway Mode"
              icon={FlaskConical}
              iconColor={testMode ? 'text-gray-200' : 'text-green-600'}
              iconBg={testMode ? 'bg-gray-200' : 'bg-green-50'}
            >
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 mt-2">
                  ...
                  >
                    ...
                    {testMode && (
                      <Radio size={16} className="ml-auto text-gray-200 shrink-0" />
                    )}
                  </label>
                ...
                {!testMode && (
                  <InfoBanner icon={AlertTriangle} color="amber">
                    ...
                    <code className="bg-gray-200 px-1 rounded text-xs">rzp_live_</code>.
                  </InfoBanner>
                )}
              </div>
            </SectionCard>
        ...
        <ScrollReveal delay={300}>
          <div className="flex justify-end pt-2 pb-8">
            <button
              onClick={handleSave}
              disabled={isSaving}
              className="flex items-center gap-2 bg-[#C9A84C] hover:bg-[#a65d14] disabled:opacity-70 text-white px-7 py-3 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
            >
              <Save size={18} />
