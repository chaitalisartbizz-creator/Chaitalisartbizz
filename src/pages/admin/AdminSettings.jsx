...
export default function AdminSettings() {
  ...
  const handleSave = async () => {
    try {
      await axios.put('/api/settings', formData).catch(err => console.warn('API error:', err));
      if (typeof setFrontendSettings === 'function') setFrontendSettings(formData);
    ...
    <div className="max-w-4xl animate-fade-in">
      <ScrollReveal>
        <div className="flex items-center justify-between mb-8">
          ...
          <button 
            onClick={handleSave}
            className="flex items-center gap-2 bg-[#C9A84C] hover:bg-[#E06900] text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
          {[
            ...
            <Link key={item.path} to={item.path}
              className="flex items-center gap-2 bg-[#F2EDE4]/50 hover:bg-[#F2EDE4] border border-[#F2EDE4] text-[#2C2C2C] font-semibold px-4 py-3 rounded-xl transition-colors text-sm">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-4 font-semibold text-sm transition-all relative whitespace-nowrap ${
                    activeTab === tab.id ? 'text-[#C9A84C]' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
                  }`}
                >
                  <Icon size={16} /> {tab.label}
                  {activeTab === tab.id && (
                    <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#C9A84C] rounded-t-full" />
                  )}
                </button>
              );
            })}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <textarea name="footerDescription" value={formData.footerDescription || ''} onChange={handleChange}
                    rows={4}
                    className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-[#C9A84C] focus:ring-2 focus:ring-[#F2EDE4] transition-all font-medium resize-none"
