...
import { X, ArrowRight, ArrowLeft, CheckCircle2, MapPin, Phone, User, Package, Truck, ChevronDown, ChevronUp, ShieldCheck } from 'lucide-react';
...
function buildWhatsAppMessage(orderId, customerInfo, cartItems, total, paymentMethod) {
  ...
  return encodeURIComponent(
    `✨ New Fortune Food Order #${orderId}\nCustomer: ${customerInfo.name}\nPhone: ${customerInfo.phone}\nAddress: ${customerInfo.address}\n\nItems:\n${itemLines}\n\nTotal Amount: ₹${total}\nPayment Method: ${paymentMethod}`
  );
}
...
function StepProgress({ step }) {
  ...
  return (
    <div className="flex items-center justify-center gap-0 px-5 py-3 bg-amber-50/50 border-b border-amber-200">
      {steps.map((label, i) => {
        const done    = i < step;
        const active  = i === step;
        return (
          <React.Fragment key={label}>
            <div className="flex flex-col items-center gap-1">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all duration-300 ${
                ...
                active ? 'bg-[#d97706] text-white shadow-[0_0_0_4px_rgba(245,158,11,0.2)]' :
                         'bg-amber-100 text-amber-800/60'
              }`}>
                {done ? '✓' : i + 1}
              </div>
              <span className={`text-[10px] font-bold tracking-wide ${active ? 'text-[#d97706]' : done ? 'text-emerald-700' : 'text-stone-400'}`}>
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`h-0.5 w-12 mx-1 mb-3 rounded-full transition-all duration-500 ${i < step ? 'bg-emerald-500' : 'bg-amber-200'}`} />
            )}
          </React.Fragment>
        );
      })}
    ...
    <div className="rounded-2xl border border-amber-200 bg-amber-50/80 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {toFree > 0 && (
            <span className="flex items-center gap-1 text-[10px] font-bold text-amber-900 bg-amber-200/80 px-2 py-0.5 rounded-full">
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="px-4 pb-3 space-y-1.5 border-t border-amber-200/60 pt-2">
              {cartItems.map(item => (
              ...
              <div className="border-t border-amber-200 pt-1.5 space-y-0.5">
                ...
                <div className="flex justify-between text-sm font-black text-stone-900 pt-1 border-t border-amber-200">
...
function OrderSummary({ cartItems, cartTotal }) {
  ...
  return (
    <div className="bg-amber-50/80 border border-amber-200 rounded-2xl p-4 space-y-2.5">
      <p className="text-xs font-bold text-stone-800 uppercase tracking-wide font-cinzel">Fortune Food Summary</p>
      ...
      <div className="border-t border-amber-200 pt-2 space-y-1">
        ...
        <div className="flex justify-between text-sm font-black text-stone-900 pt-1 border-t border-amber-200">
          <span>Grand Total</span><span className="text-[#5c3110] font-extrabold text-base">₹{total}</span>
        </div>
      </div>
    </div>
  );
...
export default function CheckoutModal({ isOpen, onClose, cartItems, cartTotal, onOrderSuccess }) {
  ...
  async function placeCOD() {
    ...
    try {
      ...
      const orderId = res.data?.orderId || res.data?.order?.id || res.data?.id || `FORTUNE${Date.now()}`;
  ...
  async function placeOnline() {
    ...
    try {
      ...
      const options = {
        ...
        name: 'Fortune Food',
        description: 'Organic Dry Fruits & Spices Order', order_id: rpOrder.id,
        ...
        handler: async (response) => {
          try {
            ...
            const orderId = saveRes.data?.orderId || saveRes.data?.order?.id || saveRes.data?.id || `FORTUNE${Date.now()}`;
      ...
      >
        ...
        <div className="bg-gradient-to-r from-[#5c3110] via-[#d97706] to-[#b45309] px-5 py-4 flex items-center justify-between flex-shrink-0 shadow-md">
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-amber-50 font-cinzel font-bold text-base leading-tight">Fortune Food Checkout</p>
          </div>
          <button
            onClick={handleClose}
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            {step === 0 && (
              ...
              >
                ...
                <div className="space-y-3">
                  <Field label="Full Name *" icon={User} error={errors.name}>
                    <input
                      ...
                      className={`w-full border ${errors.name ? 'border-red-400 bg-red-50' : 'border-amber-200'} bg-amber-50/20 rounded-xl px-3.5 py-2.5 text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-[#d97706] transition`}
                    />
                  </Field>

                  <Field label="Mobile Number *" icon={Phone} error={errors.phone}>
                    <div className="flex gap-2">
                      <div className="flex items-center bg-amber-100/70 border border-amber-200 rounded-xl px-3 py-2.5 text-sm font-bold text-amber-900 flex-shrink-0">
                        +91
                      </div>
                      <input
                        type="tel"
                        ...
                        className={`flex-1 border ${errors.phone ? 'border-red-400 bg-red-50' : 'border-amber-200'} bg-amber-50/20 rounded-xl px-3.5 py-2.5 text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-[#d97706] transition`}
                      />
                    </div>
                  </Field>
                  ...
                  <Field label="Delivery Address *" icon={MapPin} error={errors.address}>
                    <textarea
                      ...
                      className={`w-full border ${errors.address ? 'border-red-400 bg-red-50' : 'border-amber-200'} bg-amber-50/20 rounded-xl px-3.5 py-2.5 text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-[#d97706] transition resize-none`}
                    />
                  </Field>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="Pincode">
                      <input
                        ...
                        placeholder="400001"
                        className="w-full border border-amber-200 bg-amber-50/20 rounded-xl px-3.5 py-2.5 text-sm text-stone-800 placeholder-stone-400 focus:outline-none focus:border-[#d97706] transition"
                      />
                    </Field>
                    <Field label="City">
                      <input
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-[#5c3110] via-[#d97706] to-[#b45309] text-amber-50 font-bold text-sm py-3.5 rounded-2xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2"
                >
            ...
            {step === 1 && (
              ...
              >
                <button
                  onClick={() => goTo(0)}
                  className="flex items-center gap-1 text-xs text-[#d97706] font-bold hover:underline"
                >
                  <ArrowLeft size={13} /> Edit Address
                </button>

                <div className="flex items-start gap-2 bg-amber-50/60 border border-amber-200 rounded-xl px-3.5 py-2.5">
                  <MapPin size={14} className="text-[#d97706] mt-0.5 flex-shrink-0" />
                  <div className="min-w-0">
                    <p className="text-[10px] font-bold text-amber-900 uppercase tracking-wide mb-0.5">Shipping Address</p>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`w-full text-left rounded-2xl border-2 p-4 transition-all ${
                        payMethod === opt.id
                          ? 'border-[#d97706] bg-amber-50 shadow-md'
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                          payMethod === opt.id ? 'border-[#d97706] bg-[#d97706]' : 'border-stone-300'
                        }`}>
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full bg-gradient-to-r from-[#5c3110] via-[#d97706] to-[#b45309] text-amber-50 font-bold text-sm py-3.5 rounded-2xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 disabled:opacity-70"
                >
                  {loading ? (
            ...
            {step === 2 && confirmedOrder && (
              ...
              >
                ...
                <div className="bg-amber-50/70 border border-amber-200 rounded-2xl w-full px-4 py-3.5 text-left space-y-2">
                  {[
                    ...
                    <div key={row.label} className="flex justify-between text-xs text-stone-600 items-start gap-2">
                      <span className="font-bold text-amber-900 flex-shrink-0 w-20">{row.label}</span>
                      <span className="text-right flex-1 font-medium">{row.value}</span>
                    </div>
                  ))}
                  <div className="border-t border-amber-200 pt-2 flex justify-between text-sm font-black text-stone-900">
                    <span>Total Amount</span>
                    <span className="text-[#5c3110] font-extrabold">₹{grandTotal(cartTotal)}</span>
                  </div>
                </div>

                <p className="text-stone-500 text-xs">
                  Thank you for choosing Fortune Food. We are dispatching your 100% pure organic shipment! ✨
                </p>
